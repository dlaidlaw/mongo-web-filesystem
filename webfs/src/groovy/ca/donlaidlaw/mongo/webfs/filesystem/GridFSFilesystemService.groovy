...
import ca.donlaidlaw.mongo.webfs.AccessDeniedException
import ca.donlaidlaw.mongo.webfs.EntityValidationException
import ca.donlaidlaw.mongo.webfs.FileAccessDeniedException
import ca.donlaidlaw.mongo.webfs.FileMetadata
import ca.donlaidlaw.mongo.webfs.FileMetadataMapper
import ca.donlaidlaw.mongo.webfs.FileNotFoundException
import ca.donlaidlaw.mongo.webfs.InvalidMD5ChecksumException
import ca.donlaidlaw.mongo.webfs.WebFSFile
...
class GridFSFilesystemService implements FilesystemService {
	...
	Result<FileMetadata> searchFiles(FileSearchConditions conditions, Page page) {
		...
		// check and add to results
		for (GridFSFile file in files) {
			try {
				// add only if user has access to file
				checkFileAccess(file, conditions.tenant)
				results.list.add(mapper.read(file))
			} catch (Exception e) {
				// skip it - can't include un-accessible file to results
			}
		}
